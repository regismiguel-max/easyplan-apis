...
export const addCampaignToQueue = async (data: SendEmailDTO) => {
  ...
  console.log(`🔑 JOB: Adicionando job para campanha ${data.baseData.id} com hash ${contentHash}`);
  ...
  const redisKey = `campaign:${data.baseData.id}:chunk:${data.chunkIndex}:status`;
