...
export default class EditEmailCampaignUseCase implements IEditEmailCampaingUseCase {
    ...
    async execute(dto: CRUDEmailCampaignDTO): Promise<EditResponse> {
        // Transformar em entidade
        const emailCampaignEntity = EmailCampaignFactory.createFromPersistence(dto);
        ...
        if (dataToUpdate.filters) {
            ...
            const {filterResults, whereClause} = await this.filterService.processFiltersToEdit(emailCampaignEntity.emailCampaignBaseInformations.id, activeFiltersKey, activeFiltersValues);
        ...
        if(response.notRecipientGroup) {
            ...
            const deleteResponse: string = await this.emailCampaignRepository.deleteRecipientsGroup(emailCampaignEntity.emailCampaignBaseInformations.id);
        ...
        if(response.recipientGroup && Array.isArray(response.recipientGroup) && response.recipientGroup.length > 0) {
            ...
            const savedResponse: RecipientGroup[] = await this.emailCampaignRepository.saveRecipientsGroup(response.recipientGroup, emailCampaignEntity.emailCampaignBaseInformations.id);
